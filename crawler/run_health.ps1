...
# 러너에게 보고하는 것은 '체크가 돌았는가'다. HIGH 발견(1)은 체크가 제 일을 한 것 —
# 알림은 health_check가 이미 텔레그램으로 보냈고, 여기서 1을 올리면 러너/워치독이
# '잡 실패'로 오인해 다음 실행 때까지 🚨를 유지한다 (2026-07-21 오탐 8건 사고).
# 2(크롤 산출물 없음)와 3+(체커 크래시)만 진짜 실패로 올린다.
if ($code -eq 1) { exit 0 }
exit $code
