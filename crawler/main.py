...
# 표 머리가 열리는 자리 — 칸 이름이 둘 이상 잇달아 나오면 열거가 끝나고 표가 시작된 것이다.
# '… 특별연습 참여 구분 정기연습 일정 정기연습 시작일 정기연습 장소 제물포구 구립 …'
# 담당업무는 '참여'까지고 그 뒤는 연습일정 표다 (제물포, 2026-08-23).
# 표의 첫 칸 이름은 정해져 있다시피 하다(구분·연번·성명…) — 아무 칸 이름으로나 끊으면
# '가락악기를 이용한 … 지도' 같은 멀쩡한 열거를 중간에서 자른다. 첫 낱말은 좁게 잡고,
# 그 뒤에 다른 칸 이름이 잇달아 나올 때만 표로 본다.
_HEAD_OPEN = r"구분|연번|순번|성명|비고"
_HEAD_W = (r"구분|세부\s*내용|비고|일정|장소|시작일|인원|분야|과목|성명|대상|요일|시수"
           r"|연번|순번|직책|근무\s*시간|근무\s*기간")
_TABLE_HEAD_RUN = re.compile(rf"\s(?:{_HEAD_OPEN})(?:\s+\S+){{0,3}}\s(?:{_HEAD_W})(?:\s|$)")
# 담당업무 값의 꼬리 — 한국 공고의 담당업무는 문장이 아니라 이런 말로 끝나는 명사구다
_DUTY_TAIL = re.compile(r"(?:참여|지도|반주|연주|운영|관리|보조|수행|담당|준비|기획|교육"
                        r"|연습|진행|지원|공연|강의|수업|업무)\s*(?:등)?\s*$")


# 표의 칸 이름만으로 이뤄진 값은 담당업무가 아니다 — 이 낱말들만 이어진 조각을 기각한다.
_HEADER_ONLY = re.compile(
    r"(?:(?:모집|채용|선발)?\s*(?:분야|인원|기간|시간|사유|직종|과목|교과|순번|연번|비고|구분"
    r"|학교급|근무형태|근무시간|근무기간|계약기간|근로계약기간|주요업무|담당업무|수업시수"
    r"|총시수|자격면허|프로그램\s*내용|성명|직위|비\s*고)\s*){2,}")


def _duty_from_seg(seg):
    """담당업무 라벨 뒤 구획 → 값. 못 고르면 None."""
    # 다음 번호 항목·다른 구획에서 끊는다
    # 근무시간·제출장소·제출방법은 교육청 메타표에서 담당업무 바로 다음 칸이다. 콜론이
    # 없어 _FIELD_STOP 이 못 잡고 '담임 및 문화체육기획 근무시간 공고문 참조'가 돼
    # QC 에 버려졌다 — 표가 뻔히 값을 갖고 있는데 [미추출]이었다 (대전전자디자인고, 2026-09-11).
    seg = re.split(r"\s\d\s*\.\s|근무\s*조건|근무\s*시간|지원\s*자격|응시\s*자격"
                   r"|제출\s*서류|제출\s*장소|제출\s*방법|첨부\s*파일|전형|보수|접수", seg)[0]
    m_tab = _TABLE_HEAD_RUN.search(seg)
    if m_tab:
        seg = seg[:m_tab.start()]
    ...
    # 조사로 시작하는 조각은 문장 한복판을 자른 흔적이다 — '는 상세하게 작성합니다'
    # (울산시립합창단 원서 서식에서 나왔다, 2026-08-23). 값이 아니라 부스러기다.
    # 단 '가 . 창의음악활동 …' 의 '가'는 조사가 아니라 항목 기호다(가·나·다) — 뒤에
    # 마침표가 오면 건드리지 않는다. 이걸 빼먹어 대곡초 담당업무가 통째로 날아갔었다.
    parts = [p for p in parts
             if not re.match(r"(?:은|는|이|가|을|를|의|에|와|과|로|으로|도|만)(?!\s*\.)[\s,]", p)]
    # 표를 편 조각은 '한국어 문장인가'로 가려낸다 — 다만 담당업무는 원래 명사로 끝나서
    # ('…특별연습 참여', '…파트연습 지도') 문장 검사만 걸면 멀쩡한 값까지 버린다.
    # 제물포의 담당업무가 사흘 내리 [미추출]이던 이유가 이것이다 (2026-08-23).
    # 문장이 아니어도 '업무를 가리키는 말'로 끝나면 받는다 — 표를 편 줄은 칸 이름
    # ('… 정기연습 장소', '… 근무시간')으로 끝나므로 여기 걸리지 않는다.
    parts = [p for p in parts
             if not (len(p.split()) >= 4
                     and not _KOREAN_SENTENCE.search(p)
                     and not _DUTY_TAIL.search(p))]
    # '모집분야 및 인원 : 첼로 강사 1명' 처럼 라벨의 뒷말이 값 머리에 남는다 — 떼고 본다
    parts = [re.sub(r"^(?:및|등)\s+(?:인원\s*[:：]?\s*)?", "", p).strip() for p in parts]
    parts = [p for p in parts if 4 <= len(p) <= 60
             ...
             # 칸 이름만 늘어놓은 값 — '채용인원 주요업무 근로계약기간'. 표 머리에서 잘린
             # 조각이라 정보가 0인데, 낱말이 셋뿐이라 위의 '한국어 문장' 검사(4낱말 이상)를
             # 빠져나가고 QC 의 '헤더 낱말 연쇄'(3연속) 목록에도 없었다 (2026-09-11 A/B).
             and not _HEADER_ONLY.fullmatch(p)]
...
def _find_duty(text):
    ...
    # '담당업무'가 정확한 라벨이고 '모집분야'는 대개 표를 여는 구획 제목이다. 같은 자격으로
    # 두면 앞에 있는 구획 제목이 이겨 표를 통째로 물어 온다 — extract_fields 도 같은
    # 우선순위를 쓴다 (2026-08-22). 제물포가 이것 때문에 사흘째 [미추출]이었다.
    for pat in (r"(?:담당\s*업무|담당\s*분야)", r"(?:모집\s*분야)"):
        m = re.search(pat + r"\s*[:：]?[ \t]*", text)
        if not m:
            continue
        # 서울일자리포털의 표 머리 '과목 (담당업무)' — 라벨이 괄호로 닫히는 자리다. 여기서
        # 물면 값이 ') 오케스트라(클라리넷) 채용기간 …' 꼴이 돼 QC 가 '괄호 미폐합'으로
        # 버렸고, 그래서 담당업무가 매일 [미추출]로 잡혔다 (2026-08-20). 이 꼴은
        # extract_fields 의 _SUBJECT_DUTY_CELL 이 제대로 뽑으므로 여기서는 손을 뗀다.
        # (뒤의 매치로 넘어가 보기도 했는데, 원서 서식의 '담당업무가 명시된 경우…' 같은
        #  산문을 물어 와 양주 카드를 망쳤다. 라벨 종류마다 첫 매치만 본다.)
        if text[m.end():m.end() + 1] in (")", "）"):
            continue
        v = _duty_from_seg(text[m.end():m.end() + 260])
        if v:
            return v
    return None
...
_EXCERPT_SKIP = re.compile(
    ...
    # '메일' 하나로 막으면 '접수: …이메일 제출' 같은 알짜 줄이 통째로 날아간다 —
    # 본문 공백의 원인이었다 (2026-08-19). 메뉴로 쓰인 꼴만 막는다.
    r"|첨부 ?파일|>|메일 ?(?:보내기|쓰기)|^메일$|주소복사|프린트|top|TOP|서포터즈|소식|공지사항|보도자료|서식"
    ...
    # 위 목록을 빠져나간 변형들 — 낱말이 조사·수식어로 벌어지면 안 걸렸다 (2026-08-19 실측:
    # 게시 28건 중 8건이 '다만, 필수항목에 대한 동의를 거부할 경우…' 한 문장으로 오염).
    r"|동의를? ?거부|보유 ?및 ?이용 ?기간|필수 ?항목|구직활동 ?서비스|제공 ?목적|고유식별정보"
    r"|민감정보|개인 ?정보|정보주체|열람·?정정|파기 ?절차"
    r"|\[채용공고\]|\[공지\]|\[입찰\]|\[결과\]|\[알림\]"
...
def _refill_from_raw(items, today):
    ...
    for it in items:
        ...
        if not it.get("bodyExcerpt"):
            ...
            # 글머리표도 줄 경계다 — 기독정보넷 본문은 '■ 모집분야 : … ■ 사 례 비 : 월 40만원 …'
            # 처럼 한 줄에 다 이어 붙어 있어 90자 상한에 걸려 통째로 버려졌다 (2026-08-22).
