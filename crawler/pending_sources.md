...
## 오케스트라 커버리지 공백 — 접근 실패분 (2026-08-28 정찰)

사용자 제보("프로 오케스트라 공고가 하나도 안 올라온다")로 전수 조사한 결과.
인천·성남·강릉은 이날 generic 소스로 등록했고, 아래는 붙이지 못한 곳들이다.

- **경기아트센터(경기필하모닉)** ggac.or.kr — 무거운 SPA. jsfetch 5초 렌더로도 앵커
  11개뿐(내비만). 채용 게시판 실주소를 브라우저에서 사람이 확인해 와야 한다.
- **청주시립예술단** — cheongju.go.kr/cjart 404. 예술단 사이트 주소 자체를 다시 찾아야 함.
- **수원시립예술단** artsuwon.suwon.go.kr — requests·curl 둘 다 접속 실패. TLS 차단이면
  TLS_BLOCKED_HOSTS 등재 시도, 아니면 도메인이 바뀐 것.
- **김천시립예술단** gimcheon.go.kr/culture — 접속 실패(호스트 오류 추정).
- ~~과천시립예술단~~ → 이미 `g_gcart`(gcart.or.kr)로 커버 중이었다 — gccf 를 헛짚은 것.
- ~~구미시립예술단~~ → `g_gumi` 가 **2026-07-11 ba0c548 의 generic_sources.json 통째
  덮어쓰기로 유실**돼 48일간 빠져 있었다(CLAUDE.md 함정 그대로). 2026-08-28 git 이력에서
  복구. gumi.go.kr/culture 500 은 무관한 다른 주소.
- **진주시립예술단** — jinju.go.kr 시청 통합 채용공고(00130/02730/06660.web)에 섞여
  나오는데 행에 날짜가 없다. 시 전체 공고라 노이즈도 커서 보류. 예술단 전용 게시판을 찾을 것.
- **제주도립예술단** — 도청 통합 시험·채용(works/exam.htm)뿐. 노이즈 커서 보류.
- 부산(bscc)·천안(cfac)·안양(ayac)·대구(대구문화예술회관)·울산·창원(cwcf)·포항(phcf)은
  이미 운영 주체 소스로 커버 중 — 새로 볼 필요 없음.

## 레슨인포 (lessoninfo.co.kr) — 판정: 크롤 금지 (2026-09-24)

사용자 경유 요청("피아노 전공생들이 많이 보는 곳"). 정찰 결과:
- 게시판: /music-jobs(피아노강사·음악학원) /institution-jobs(음악단체·기관) /worship-jobs(종교계)
  /culture-jobs 등. 범위상 '교육 — 취미·입문' 강사 공고가 대량(유량 큼).
- robots.txt: 목록·상세(`/board/board.php`)는 허용. 그러나 **연락처는
  `/board/views/_load/contact_get.php?wr_no=` XHR 로만 오고 `/board/views/` 는 Disallow.**
  로그인 게이트 문구도 있음 → 연락처 없는 제목·링크 미러링만 가능.
- 이용약관 15조: "레슨인포의 서비스 정보를 이용하여 얻은 정보를 사전 승낙 없이 복제 또는
  유통시키거나 상업적으로 이용하는 경우" 를 이용 제한 사유로 명시.
- 목록 페이지의 /music-jobs 등 예쁜 주소는 같은 HTML 을 주고 실제 행에 날짜가 없음 —
  목록 자체도 JS 렌더 의심(robots·약관 때문에 더 파지 않음).
- 결론: CLAUDE.md '상용 마켓플레이스 = 크롤 금지' 그대로. 살릴 길은 사이트의 **제휴문의**
  창구로 피드/API 를 받는 것. 종교계 게시판은 기독정보넷(cjob)과 겹쳐 손실이 작다.
