...
- **집계 포털이 준 '원문 링크'가 게시판 목록일 수 있다** — 광신대 강사 초빙의 officialUrl 이
  ...
  `_find_detail_in_list` 로 상세를 찾아 들어간다. **상세 링크가 `javascript:` 면 주소가 아니라
  함수 호출이라 자동 교정이 원리적으로 불가능하다** — 그런 건 `originIsList` 로 표시해
  마감 미확인 알림에 '링크가 목록이라 바로 안 열린다'고 적어 헛걸음을 막는다.
